<?php 
	...
	#第一种循环
/* 	$id=$_GET["id"];//接受的参数:	1,2,3,4
	// 遍历数组,保留符合条件的数据
	// 数据中的pid的值与参数$id的值一样
	$result = array();					//定义一个空数组,保存符合条件的数据
	$num = count($list);				//获取$list的长度
	for($i=0;$i<$num;++$i){			//循环遍历
		if($list[$i]['pid']==$id){//判断每一个元素的pid的值是否与$id相等
				$result[] = $list[$i]; //将该元素方式$result中
		}
	}
	echo json_encode($result);	//将$result数组返回
	 */
